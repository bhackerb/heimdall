...
from heimdall.scanner import PendingUpdate, ScanResult, UpdateType


def is_held(update: PendingUpdate, config: HeimdallConfig) -> bool:
...
def in_maintenance_window(config: HeimdallConfig) -> bool:
    """Check if we're currently in the maintenance window."""
    now = datetime.now()
    return now.hour == config.policy.maintenance_hour


def get_auto_applicable(scan: ScanResult, config: HeimdallConfig) -> list[PendingUpdate]:
...
def should_auto_apply_now(config: HeimdallConfig) -> bool:
...
def classify_scan(scan: ScanResult, config: HeimdallConfig) -> dict[str, list[PendingUpdate]]:
