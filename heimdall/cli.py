...
def cmd_scan(args: argparse.Namespace, config: HeimdallConfig) -> int:
    """One-shot update + security scan, print results."""
    from heimdall.scanner import full_scan
    from heimdall.security import full_security_scan

    print(f"Heimdall scanning {config.hostname} ({config.role})...\n")
    ...
    if scan.updates:
        for u in scan.updates:
            ...
            print(f"  {u.source}: {u.package} {u.current_version} -> {u.new_version}{marker}")
...
def cmd_status(args: argparse.Namespace, config: HeimdallConfig) -> int:
    """Show current state — pending updates, security posture."""
    from heimdall.scanner import full_scan, check_reboot_required
    from heimdall.security import full_security_scan

    print(f"Heimdall v{__version__} — {config.hostname} ({config.role})")
...
def cmd_apply(args: argparse.Namespace, config: HeimdallConfig) -> int:
    """Apply security updates."""
    from heimdall.scanner import full_scan
    from heimdall.policy import get_auto_applicable, classify_scan
    from heimdall.updater import apply_updates
...
def cmd_report(args: argparse.Namespace, config: HeimdallConfig) -> int:
    """Send full report to Mithrandir."""
    from heimdall.scanner import full_scan
    from heimdall.security import full_security_scan
    from heimdall.reporter import send_report
...
def cmd_daemon(args: argparse.Namespace, config: HeimdallConfig) -> int:
    """Run as background daemon — scan on schedule, auto-apply security."""
    from heimdall.scanner import full_scan
    from heimdall.security import full_security_scan
    from heimdall.policy import get_auto_applicable, should_auto_apply_now
    from heimdall.updater import apply_updates
    from heimdall.reporter import send_report

    print(f"Heimdall daemon starting — {config.hostname} ({config.role})")
...
def cmd_init(args: argparse.Namespace, config: HeimdallConfig) -> int:
    ...
    if engram_secret:
        content = content.replace('engram_secret: ""', f'engram_secret: "{engram_secret}"')

    CONFIG_PATH.write_text(content)
    print(f"\nConfig written to {CONFIG_PATH}")
    print("Edit it to fine-tune settings, then run: heimdall scan")
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        prog="heimdall",
        description="Heimdall — lightweight security & update agent",
    )
    parser.add_argument(
        "--version", action="version", version=f"heimdall {__version__}",
    )
    parser.add_argument(
        "--config", type=Path, default=None,
        help="Path to config file (default: ~/.config/heimdall/config.yaml)",
    )
